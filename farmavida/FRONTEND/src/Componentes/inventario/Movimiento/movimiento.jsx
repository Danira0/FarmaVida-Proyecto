...
function Movimiento() {
  ...
  const [filtroEntradas, setFiltroEntradas] = useState(false);
  ...
  const getPendientesDevolucion = async () => {
    try {
      const response = await api.get(
        "/movimientos/devoluciones/pendientes"
      );
  ...
  const filtrarMovimientosPorProducto = (termino) => {
    if (!termino) return Movimiento;

    return Movimiento.filter((mov) =>
      mov.producto.toLowerCase().includes(termino.toLowerCase())
    );
  };
                      ...
                      <tbody className="table-group-divider">
                        {Movimiento.map((mov) => (
                          <tr key={mov.ID_STOCK}>
                            ...
                            <td className="table-info">{mov.unidad_por_blister}</td>
                        ...
                        <form onSubmit={handleSubmit}>
                          ...
                          {tipo_movimiento === "Devolucion" ? (
                            ...
                            <>
                              ...
                              {/* Campos específicos según presentación y tipo de movimiento */}
                              {PresentacionID_Presentacion && (
                                <>
