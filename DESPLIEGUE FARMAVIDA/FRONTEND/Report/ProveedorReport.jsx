...
// 4. Reporte general de proveedores (Laboratorios)
  ...
  <BaseReport title="INFORME GENERAL DE PROVEEDORES" data={data}>
    <View style={styles.table}>
      <View style={[styles.tableRow, styles.tableHeader]}>
        ...
        <View style={[styles.tableCol, { width: '40%' }]}><Text>Dirección</Text></View>
      </View>
      {(Array.isArray(data) ? data : []).map((laboratorio) => (
        <View key={laboratorio.id || laboratorio.nombre} style={styles.tableRow}>
          <View style={[styles.tableCol, { width: '15%' }]}><Text>{formatValue(laboratorio.id)}</Text></View>
          <View style={[styles.tableCol, { width: '15%' }]}><Text>{formatValue(laboratorio.nombre)}</Text></View>
          <View style={[styles.tableCol, { width: '30%' }]}><Text>{formatValue(laboratorio.telefono)}</Text></View>
          <View style={[styles.tableCol, { width: '40%' }]}><Text>{formatValue(laboratorio.direccion)}</Text></View>
        </View>
      ))}
    </View>
  </BaseReport>
...
LaboratoriosReport.propTypes = {
  data: PropTypes.array.isRequired,
};

// 5. Reporte detalle de laboratorio/proveedor
export const LaboratorioDetalleReport = ({ data }) => {
  if (!Array.isArray(data) || data.length === 0) {
    return (
      <Document>
        <Page size="A4" style={styles.page}>
          <View style={styles.section}>
            <Text>No hay datos disponibles para mostrar el detalle del laboratorio/proveedor</Text>
          </View>
        </Page>
      </Document>
    );
  ...
  const laboratorio = data[0]; // se asume que es solo 1

  return (
    <BaseReport title={`DETALLE DE LABORATORIO: ${formatValue(laboratorio.nombre)}`} data={data}>
      <View style={styles.table}>
...
LaboratorioDetalleReport.propTypes = {
  data: PropTypes.array.isRequired,
};

// 6. Reporte de productos por fecha de ingreso
