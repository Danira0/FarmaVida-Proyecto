...
function Emplecrud() {
  ...
  const getEmpleados = async () => {
    try {
      const respuesta = await api.get("/obtener/empleado");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (operacion === 1) {
        await api.post("/registrar/empleado", empleado);
      } else {
        await api.put(`/actualizar/empleado/${ID_Usuario}`, empleado);
      }
      getEmpleados();
      closeModal();
      Swal.fire({
  ...
  const deleteEmpleado = async (ID_Usuario, nombre) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          await api.delete(`/eliminar/empleado/${ID_Usuario}`);
          getEmpleados();
          Swal.fire({
            icon: "success",
            title: "Eliminado",
      ...
      <div className="container_prove">
        <div className="op row d-flex">
          ...
          <div className="col-lg-10 col-md-5 mt-0">
            ...
            {showForm && (
              <div id="modalEmpleado" className="modal-fade" style={{ display: 'block' }}>
