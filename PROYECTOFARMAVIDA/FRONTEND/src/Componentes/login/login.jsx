...
import HomeAdmi from "../home/homeadmi";
import HomeEmple from "../home/homeemple";
...
function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.nombre_usuario || !formData.contrasena_usuario || !formData.requestedRole) {
      setMessage({ text: "Por favor, complete todos los campos y seleccione un rol.", type: "warning" });
      setIsLoading(false);
      return;
    }
    ...
    try {
      ...
      if (response.data.token) {
        ...
        setRedirectMessage(`Redirigiendo al panel de ${user.rolId === 1 ? 'Administrador' : 'Empleado'}...`);
        setShowRedirect(true);
        
      } else {
        setMessage({ 
          text: response.data.message || "Usuario, contraseña o rol incorrectos.", 
          type: "danger" 
        });
      }
    } catch (error) {
        ...
        <div className="row border rounded-5 p-3 bg-white shadow box-area">
          ...
          <div className="col-md-6 right-box">
            <div className="row align-items-center">
              ...
              <div className="btns">
                <button
                  className={`btn btn-lg fs-6 ${
                    formData.requestedRole === "1" ? "btn-success" : "btn-primary"
                  }`}
                ...
                <button
                  className={`btn btn-lg fs-6 ms-2 ${
                    formData.requestedRole === "2" ? "btn-success" : "btn-primary"
                  }`}
              ...
              {message.text && (
                <Alert variant={message.type} className="mt-3" dismissible onClose={() => setMessage({ text: "", type: "" })}>
                  {message.text}
                </Alert>
              )}
