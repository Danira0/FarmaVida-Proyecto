    ...
    <Document>
      <Page size="A4" style={styles.page}>
        ...
        <View style={styles.section}>
          ...
          {filtros && (
            <Text style={styles.subtitle}>
              Filtros aplicados: {Object.entries(filtros).map(([key, value]) => (
                `${key}: ${value}`
              )).join(' | ')}
            </Text>
          )}
        ...
        <View style={styles.footer}>
          <Text>Total registros: {data.length} | Generado por: Sistema de Gestión de Usuarios</Text>
        </View>
      </Page>
    </Document>
  );
        ...
        {data.map((usuario, index) => (
          <View key={index} style={styles.tableRow}>
            <View style={[styles.tableCol, { width: '15%' }]}>
              <Text>{formatValue(usuario.ID_Usuario)}</Text>
            </View>
            <View style={[styles.tableCol, { width: '20%' }]}>
              <Text>{formatValue(usuario.nombre)}</Text>
            </View>
            <View style={[styles.tableCol, { width: '20%' }]}>
              <Text>{formatValue(usuario.apellido)}</Text>
              </View>
            <View style={[styles.tableCol, { width: '30%' }]}>
              <Text>{formatValue(usuario.email)}</Text>
            </View>
            <View style={[styles.tableCol, { width: '20%' }]}>
              <Text>{formatValue(usuario.rol_descripcion)}</Text>
            </View>
          </View>
        ))}
      </View>
...
export const UserByReport = ({ data, filtros }) => {
  ...
  return (
    <BaseReport title="INFORME DETALLADO DE USUARIO" data={data} filtros={filtros}>
      <View style={{ marginBottom: 20 }}>
        ...
        <View style={styles.table}>
          ...
          {[
            ...
            { label: 'Rol', value: user?.rol_descripcion || 'N/A' },
...
export const UserRolReport = ({ data, filtros }) => {
  const validData = Array.isArray(data) ? data.filter(user => user && typeof user === 'object') : [];
  const rolDescripcion = validData[0]?.rol_descripcion || filtros?.rolDescripcion || 'N/A';

  return (
    <BaseReport 
      title={`USUARIOS POR ROL: ${rolDescripcion}`} 
      data={validData} 
      filtros={filtros}
    >
      <View style={styles.summary}>
        <View style={styles.summaryItem}>
          ...
          <Text style={styles.summaryValue}>{validData.length}</Text>
        </View>
        <View style={styles.summaryItem}>
      ...
      <View style={styles.table}>
        ...
        {validData.map((user, index) => (
          <View key={index} style={styles.tableRow}>
            ...
            <View style={[styles.tableCol, { width: '20%' }]}><Text>{formatValue(user?.correo)}</Text></View>
            ...
            <View style={[styles.tableCol, { width: '15%' }]}>
              <Text style={user?.total_movimientos > 0 ? styles.entradaText : null}>
