...
export const getDevolucionesPendientes = async (req, res) => {
  ...
  try {
    const [result] = await db.promise().query(`
      SELECT 
        ...
        IFNULL(SUM(d.cantidad), 0) as cantidad_devuelta,
        m.cantidad - IFNULL(SUM(d.cantidad), 0) as cantidad_pendiente
      FROM movimiento m
...
export const createDevolucion = async (req, res) => {
  const { movimientoId } = req.params;
  const { motivo, observaciones, cantidad } = req.body;
  ...
  try {
    ...
    // Usamos el cálculo directo de la base de datos para mayor precisión
    const horasTranscurridas = movimientoOriginal.horas_transcurridas_db || 
                              (ahora - fechaMovimiento) / (1000 * 60 * 60);
    
    if (horasTranscurridas > MAX_HORAS_DEVOLUCION) {
      return res.status(400).json({
        message: `No se puede devolver el producto. Han pasado más de ${MAX_HORAS_DEVOLUCION} horas (1 día) desde la salida.`,
        error: "TIEMPO_EXCEDIDO",
        horasTranscurridas: horasTranscurridas.toFixed(2),
        fecha_salida: movimientoOriginal.fecha_hora_movimiento,
        fecha_intento_devolucion: ahora.toISOString()
      });
    }
    ...
    if (cantidadDevolucion <= 0) {
      return res.status(400).json({
        message: "No hay cantidad disponible para devolver",
        error: "SIN_STOCK_DEVOLUCION",
        cantidad_disponible: cantidadDisponibleDevolver
      });
    }
    
    if (cantidadDevolucion > cantidadDisponibleDevolver) {
      return res.status(400).json({
        message: `La cantidad a devolver (${cantidadDevolucion}) excede la disponible (${cantidadDisponibleDevolver})`,
